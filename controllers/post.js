...
const createPost = async (req, res) => {
  ...
  try {
    ...
    if (!cats) {
      return res.json({ error: "Its a project or service article ?" });
    }
...
const postsServiceArticle = async (req, res) => {
  try {
    const all = await Post.find({ cats: "Service" })
      .populate("featuredImage")
      .populate("postedBy", "name")
      .populate("categories", "name slug")
      .sort({ createdAt: -1 });
...
const postsProjectArticle = async (req, res) => {
  try {
    const all = await Post.find({ cats: "Project" })
      .populate("featuredImage")
      .populate("postedBy", "name")
      .populate("categories", "name slug")
      .sort({ createdAt: -1 });
...
const editPost = async (req, res) => {
  try {
    ...
    const { title, content, featuredImage, categories, cats, checked } =
      req.body;
    // get category ids based on category name
    let ids = [];
    for (let i = 0; i < categories.length; i++) {
